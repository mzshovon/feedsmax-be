...
class TargetByOSVersion implements RuleEngineInterface
{
    ...
    public function match(array $request, ...$args): bool
    {
        ...
        if(isset($request['OSVersion'])){
            $request_os_version = intval($this->fullVersion($request['OSVersion']));
            $rule_os_version = intval($this->fullVersion($version));

            switch ($comparator){
                case Comparator::Equal->value:
                    if($request_os_version == $rule_os_version){
                        return true;
                    }
                    break;
