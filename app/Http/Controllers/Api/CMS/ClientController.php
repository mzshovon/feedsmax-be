...
class ClientController extends Controller
{
    ...
    public function __construct(ClientServiceInterface $repo)
    {
        $this->repo = $repo;
    }
    /**
    ...
    {
        try {
            $data = $this->repo->getClientById($id);
            return $this->success($data, !empty($data) ? Response::HTTP_OK : Response::HTTP_NOT_FOUND);
        } catch (\Exception $e) {
            return $this->error($e->getMessage(), $e->getTrace(), 500);
        }
     ...
     */
    public function update(ClientUpdateRequest $request, int $clientId)
    {
        try {
            $data = $this->repo->update($request->all(), $clientId);
            return $this->success($data, !empty($data) ? Response::HTTP_OK : Response::HTTP_NOT_FOUND);
        } catch (\Exception $e) {
            return $this->error($e->getMessage(), $e->getTrace(), 500);
        }
